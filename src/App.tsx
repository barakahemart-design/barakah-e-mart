...
export default function App() {
  ...
  const runAutoRecovery = async () => {
    ...
    if (changed) {
      ...
      const successMsg = recoveredNames.length > 0 
        ? `âœ¨ Successfully recovered ${recoveredNames.length} profile(s) (${recoveredNames.slice(0, 3).join(", ")}${recoveredNames.length > 3 ? "..." : ""}) back to the CRM directory!`
        : "âœ¨ All parent directories and customer profiles completely restored!";
      triggerNotification(successMsg, "success");
    } else {
      triggerNotification("ðŸ” No profiles found for recovery. Your database is fully synchronized!", "info");
    }
  };
  ...
  const triggerCloudBackupRestore = async () => {
    ...
    try {
      ...
      if (restored) {
        ...
        triggerNotification("Cloud backup restored and synchronized successfully! ðŸŽ‰", "success");
      } else {
  ...
  const handleSignUp = async (email: string, pass: string) => {
    ...
    if (user.restored) {
      triggerNotification("Cloud backup and database restore completed successfully! ðŸŽ‰");
    } else {
  ...
  const handleSignIn = async (email: string, pass: string) => {
    ...
    if (user.restored) {
      triggerNotification("Welcome back! Your store data was seamlessly restored from cloud backup. ðŸŽ‰");
    } else {
  ...
  const [tempCurrency, setTempCurrency] = useState("à§³");
  ...
  useEffect(() => {
    if (businessInfo) {
      ...
      setTempCurrency(businessInfo.currencySymbol || "à§³");
  ...
  const handleDeleteProduct = async (id: string, name: string) => {
    if (currentPanel !== "admin") {
      triggerNotification("Security block: Only administrators are authorized to delete products! ðŸ›‘", "error");
      return;
    }
    ...
    if (dangerAction === "delete_transactions") {
      ...
      if (activeUser && !activeUser.isGuest) {
        ...
        uploadPasscodeBackup(activeUser.email, passcode, {
          ...
          triggerNotification("System database and all paired devices successfully reset! ðŸŸ¢", "success");
        }).catch(e => {
  ...
  const handleMarkNegativeSaleUpdated = (id: string, updated: boolean) => {
    ...
    triggerNotification(updated ? "Product negative stock marked as green / updated! ðŸŸ¢" : "Product negative stock reset to red.", "success");
  };
  ...
  const handleDeletePurchase = async (id: string) => {
    if (currentPanel !== "admin") {
      triggerNotification("Security block: Only administrators are authorized to delete purchases! ðŸ›‘", "error");
      return;
    }
  ...
  const handleDeleteTransaction = async (id: string) => {
    if (currentPanel !== "admin") {
      triggerNotification("Security block: Only administrators are authorized to delete transactions / sales! ðŸ›‘", "error");
      return;
    }
  ...
  const handleEditTransaction = (id: string, updatedFields: Partial<Transaction>) => {
    if (currentPanel !== "admin" && currentPanel !== "sales") {
      triggerNotification("Security block: Unauthorized action! ðŸ›‘", "error");
      return;
    }
  ...
  const banglaToEnglishDigits = (str: string): string => {
    ...
    return str.replace(/[à§¦-à§¯]/g, (m) => map[m] || m);
  };
  ...
  const runTranslationForUI = async () => {
    ...
    if (result) {
      ...
      triggerNotification("âœ¨ Customer details translated to English successfully!", "success");
    } else {
  ...
  const handleAddContact = async (e: React.FormEvent) => {
    ...
    if (cleanPhone.length !== 11) {
      triggerNotification("âš ï¸ Bangladeshi mobile phone numbers must be exactly 11 digits! Yours is " + cleanPhone.length + " digits.", "error");
      return;
    }
    ...
    if (hasBengaliCharacters(finalName) || hasBengaliCharacters(finalAddress)) {
      triggerNotification("ðŸ”„ Bengali input detected! Automatically transliterating to English...", "info");
...
`.trim();
  ...
  const handleWhatsAppShare = (pur: Purchase) => {
    const due = pur.dueAmount ?? 0;
    const isFullyPaid = due <= 0;
    const statusStr = isFullyPaid ? "âœ… FULLY SETTLED" : "âš ï¸ OUTSTANDING DUE";
    ...
    <div className="min-h-screen md:h-screen md:overflow-hidden bg-[#121214] text-white font-sans flex flex-col md:flex-row" id="terminal-layout">
          ...
          <div className="flex items-center gap-3 pb-4 border-b border-[#2D2D35]" id="sidebar-logo">
            <div className="w-10 h-10 flex items-center justify-center bg-[#00E676]/10 rounded-xl text-[#00E676] border border-[#00E676]/20 overflow-hidden shrink-0">
              {businessInfo.companyLogo && (businessInfo.companyLogo.startsWith("data:") || businessInfo.companyLogo.startsWith("http")) ? (
                ...
                <span className="font-extrabold text-sm truncate">{businessInfo.companyLogo || "âš¡"}</span>
              )}
            </div>
      ...
      <main className="flex-1 min-w-0 bg-[#070b13] flex flex-col md:h-screen md:overflow-hidden pb-20 md:pb-0" id="viewport-workspace">
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-6" id="scrollable-content-area">
          ...
          {activeTab === "reports" && (
            <ReportsView
              ...
              contacts={contacts}
              businessInfo={businessInfo}
              currencySymbol={businessInfo?.currencySymbol || "à§³"}
            />
          )}
                          ...
                          <div className="flex items-center gap-3.5">
                            ...
                            <div>
                              ...
                              <div className="text-[10px] font-mono text-[#A0A0A5] mt-0.5">
                                SKU: {item.product.sku} <span className="mx-1 text-[#2D2D35]">â€¢</span> Stock: {item.product.stock} {item.product.unit}
                              </div>
                            </div>
                          </div>
                                    ...
                                    {isDue ? (
                                      <button
                                        onClick={() => {
                                          ...
                                          triggerNotification("Supplier due bill paid & fully settled in cash successfully! ðŸŸ¢", "success");
                                        }}
                                      ...
                                      <button
                                        onClick={() => {
                                          ...
                                          const promptVal = prompt(
                                            ...
                                            `â€¢ Purchased quantity in this batch: ${pur.quantity} pcs\n` +
                                            `â€¢ Available physical stock: ${availableStock} pcs\n` +
                                            `â€¢ Maximum returnable quantity: ${maxReturnQty} pcs`
                                          );
                                ...
                                <tr key={pur.id} className={`hover:bg-slate-900/10 text-slate-300 ${isFullyPaid ? 'bg-emerald-950/5' : ''}`}>
                                  ...
                                  <td className="py-4 px-4 text-center font-sans lg:min-w-[190px]">
                                    <div className="flex flex-col gap-1.5 items-center justify-center">
                                      {isFullyPaid ? (
                                        <span className="text-[10px] text-emerald-500 font-bold font-mono">Completed Settlement &nbsp;âœ”</span>
                                      ) : (
                        ...
                        <div className="flex justify-end gap-3 font-sans">
                          <button
                            onClick={() => {
                              ...
                              triggerNotification("Success! New supplier procurement record registered and asset stock updated. ðŸŸ¢", "success");
                            }}
                                          ...
                                          <button
                                            onClick={() => {
                                              ...
                                              const promptVal = prompt(
                                                `Enter quantity of ${pur.productName} you want to return to ${pur.supplierName || 'supplier'}.\n` +
                                                `â€¢ Purchased quantity in this batch: ${pur.quantity} pcs\n` +
                                                `â€¢ Available physical stock: ${matchedProduct ? matchedProduct.stock : 0} pcs\n` +
                                                `â€¢ Maximum returnable quantity: ${maxReturnQty} pcs`
                                              );
                                              if (promptVal === null) return;
                                              const qtyToReturn = parseInt(promptVal);
                          ...
                          <div>
                            {isFullyPaid ? (
                              <span className="inline-flex flex-col items-center justify-center bg-emerald-500/10 border border-emerald-500/40 px-4 py-2 rounded-2xl text-[11px] font-extrabold text-emerald-400 tracking-wider">
                                <span className="text-base">âœ”</span> FULLY SETTLED
            ...
            <div className="space-y-6 animate-fadeIn" id="view-ledger-container">
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-4 p-4 bg-[#1E1E24] border border-slate-800/80 rounded-2xl shadow-xl" id="ledger-filter-bar">
                ...
                <div className="relative w-full lg:w-96" id="ledger-search-wrap">
                  ...
                  />
                  {ledgerSearch && (
                    <button onClick={() => setLedgerSearch("")} className="absolute right-3 top-3 text-slate-500 hover:text-white transition-all">âœ•</button>
                  )}
                </div>
              ...
              <div className="space-y-4" id="ledger-timeline-feed">
                {filteredTransactions.length === 0 ? (
                  <div className="py-16 text-center text-slate-450 italic bg-[#1E1E24] border border-slate-800 rounded-2xl flex flex-col items-center justify-center gap-3">
                    <span className="text-4xl text-slate-600">ðŸ—„ï¸</span>
                                  ...
                                  <div key={idx} className="flex justify-between items-center text-[11px] font-sans text-slate-300 py-1 border-b border-slate-800/20 last:border-b-0">
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                      ...
                                      <span className="text-[10px] text-slate-400 font-mono">
                                        (@ {businessInfo.currencySymbol || "à§³"}{(it.price ?? 0).toLocaleString()}/unit)
                                      </span>
                                    ...
                                    <div className="flex items-center gap-2">
                                      {currentPanel === "admin" && (
                                        showEdit ? (
                                          ...
                                          >
                                            Edit Cost: {businessInfo.currencySymbol}{buyCost} âœï¸
                                          </button>
                                        )
                                      )}
                          ...
                          <div className="space-y-1 text-right">
                            ...
                            <div className="grid grid-cols-2 md:block gap-x-4">
                              ...
                              {t.dueBalance > 0 ? (
                                ...
                                <p className="text-slate-500 text-[10px]">Accounts Settled âœ“</p>
                              )}
                            </div>
                          </div>
                ...
                <div className="lg:col-span-4 bg-[#0a101f]/80 border border-slate-800 p-5 rounded-2xl space-y-4" id="add-expense-panel">
                  ...
                  <form onSubmit={handleAddExpense} className="space-y-4" id="outgoing-add-form">
                    ...
                    <div className="flex items-center gap-2" id="ai-classify-outgoing">
                      ...
                      >
                        {isClassifying ? "AI Categorizing..." : "âœ¨ Predict Category with AI"}
                      </button>
                      <span className="text-[9px] text-slate-500 font-mono">Gemini automatically scans outgoâ€™s category</span>
                    </div>
                    ...
                    <div className="space-y-3 p-3.5 bg-[#050912]/85 border border-slate-800/80 rounded-xl" id="outgoing-category-selection-container">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-semibold uppercase tracking-wider font-mono text-slate-400 pl-1">Voucher Amount (à§³) *</label>
                          <input
                            type="number"
                            required
                            placeholder="0"
                    ...
                    {(() => {
                      ...
                      if (filteredExpenses.length === 0) {
                        return (
                          <div className="py-16 text-center text-slate-400 italic bg-[#1E1E24]/30 border border-slate-800 rounded-2xl flex flex-col items-center justify-center gap-3">
                            <span className="text-4xl text-slate-650">ðŸ—„ï¸</span>
                      ...
                      return filteredExpenses.map((e) => {
                        ...
                        try {
                          ...
                          if (!isNaN(d.getTime())) {
                            formattedDate = `${d.toLocaleDateString('en-US', { day: '2-digit', month: 'short', year: 'numeric' })} â€¢ 12:00 PM`;
                          }
                        } catch (err) {}
          ...
          {activeTab === "contacts" && (() => {
            ...
            const getContactStatus = (c: any) => {
              ...
              if (totalOutstandingDue > 0) {
                return { label: `Due Ledger: à§³${totalOutstandingDue.toLocaleString()}`, color: "bg-[#FF5252]/10 text-[#FF5252] border border-[#FF5252]/20 font-bold text-[10px]" };
              }
                    ...
                    <div className="space-y-1.5">
                      ...
                      {cPhone.length > 0 && cPhone.length !== 11 && (
                        <p className="text-[10px] text-rose-500 font-bold pl-1 mt-0.5 min-h-[14px]">
                          âš ï¸ Bangladeshi mobile numbers must be exactly 11 digits! (Current: {cPhone.length} digits)
                        </p>
                      )}
                    </div>
                                      ...
                                      <div key={idx} className="flex flex-col text-[10px] py-1.5 border-b border-[#2D2D35]/15 last:border-0 font-mono">
                                        <div className="flex justify-between items-start gap-2">
                                          <span className="text-slate-200 font-semibold text-wrap text-left" title={prod.name}>
                                            â€¢ {prod.name} <span className="text-slate-400 font-normal text-[9px]">(@ {businessInfo.currencySymbol || "à§³"}{prod.price.toLocaleString()}/unit)</span>
                                          </span>
                                        ...
                                        {prod.quantity > 1 && (
                                          <div className="text-[9px] text-slate-500 pl-2.5 mt-0.5">
                                            Total for {prod.name}: <span className="text-[#00E676] font-semibold">{businessInfo.currencySymbol || "à§³"}{prod.total.toLocaleString()}</span>
                                          </div>
                                        )}
                                      </div>
                                    ))}
                                ...
                                {deleteContactId === c.id ? (
                                  <div className="flex items-center gap-1 bg-rose-500/10 p-0.5 rounded border border-rose-500/20">
                                    <button
                                      onClick={async () => {
                                        if (currentPanel !== "admin") {
                                          triggerNotification("Security block: Only administrators are authorized to delete contact profiles! ðŸ›‘", "error");
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        <option value="à§³">à§³ (BDT Taka - TK)</option>
                        <option value="$">$ (USD Dollar)</option>
                        <option value="â‚¹">â‚¹ (INR Rupee)</option>
        ...
        <footer className="bg-white border-t border-slate-200 px-6 py-3 shrink-0 flex items-center justify-between text-[10px] text-slate-500 font-mono" id="terminal-footer">
          ...
          <span>Â© 2026 Barakah Bill Pro</span>
        </footer>
                      ...
                      <div key={idx} className="p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white">
                        <div className="space-y-0.5 font-sans min-w-[140px]">
                          ...
                          <span className="text-[9px] text-slate-500 font-mono block">
                            Catalog rate: {businessInfo.currencySymbol || "à§³"}{(matchedProd ? matchedProd.sellPrice : (item.price ?? 0)).toLocaleString()}/unit
                          </span>
                        </div>
                        <div className="flex items-center gap-3 flex-wrap">
                          <div className="flex items-center gap-1.5">
                            ...
                            <div className="relative">
                              <span className="absolute left-1.5 top-1 text-slate-400 font-mono text-[10px]">{businessInfo.currencySymbol || "à§³"}</span>
                              <input
                                type="number"
                                min={0}
                                step="any"
            ...
            <div className="bg-[#1E1E24] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden max-w-md w-full p-6 space-y-4 text-slate-200 animate-scaleIn" id="modal-delete-expense">
              ...
              <div className="flex items-center gap-3 pt-3 font-sans">
                <button
                  onClick={async () => {
                    if (currentPanel !== "admin") {
                      triggerNotification("Security block: Only administrators are authorized to delete expense vouchers! ðŸ›‘", "error");
          ...
          <div className="bg-[#1E1E24] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden max-w-md w-full p-6 space-y-4 text-slate-200 animate-scaleIn" id="modal-edit-expense">
            ...
            <form onSubmit={handleUpdateExpenseSubmit} className="space-y-4 text-xs font-sans">
              ...
              <div className="flex items-center gap-3 pt-3 font-sans">
                ...
                <button
                  type="button"
                  onClick={xœì\ërÛ6þŸ§@ÙNâtBI–­´I£ìÈ²“zÖ—lì6»“ñL ’XS€‘U×/²ÿöÕöIö ¼ˆ$@K¬¬lä1=“P$âœóˆƒ­§¨ûq"\OxÁèà*$'[AäûOo!íp|Ìù	ž®5ôÉ•½Â™üçÊÞEƒ‘Í},ˆýc«…Æô3a/³K?À%A®Dòs'ýyÅÑp™L¼õ]Äh¸Äµ¯|4¶·á)†c£}9ã”Ù!õAXÜ‡CÔy†„9˜ÙÆ¹jì©çfid¼6Ö÷)'ÚõWÍA$Ê-^5]ïóëGŠ×†”MòO&Ow]7¿G½ý}trðõ{çoOßÿŸî÷ŽÐ÷Í”û×0lDÙì˜ºØ?ù ÇÑÖ¼Wè´ ïŠ¸È)
»…þ°?>o]HÑ|ü¶åÀ¹h¾è 0Èe4´~Äl>AR˜ÈdÂSvþqágéÏÐÞµ
´•^«Þ°}°}ÐÞ½È„Ø)(!$ÿå ÂÇØ¥Sõˆ„ÊÐ‡cÏu¾	¾²§rTS{@„ww±CìÀ,£6ôƒo"Ï¹ƒ}rXÈs»ÖDòÊÆ®k§Ü³Š"Ô¸&éOI1%0Ž?:%áÀn7:–†‘ñNþ¸Ã9Ã
ï9ÔÊ@J­ƒç?<¿0Hd„C»·w=ú¸L”zë9å_;ŽS®‘	aØwíÝVËBM½qÏuÑ	™¢Ä ~Â<ýãÚXQÖ'ú¾ç\v¯·”­¹6è@µ_ùÖûœ<ýÉü82m!ÉÜ²¬ª'Ÿc6""¶núó77úèsœÁÎ•¬š”~Šu”G&p¸d«ÊfÈ ºg“Ù(M’´H¥Ö48‹O€@H…DH#dä3€nŸqä‹-³p–Ö›LÀÜtQP”HCÞ1µó†hë›¤ÙSÄˆˆXPñ‰Q˜tëÞðÇ\Â·ÒžVáé ÜvK’$ÉýØh(òž¥ƒ¿0AÁ8"GÇêv»È"à=­ê±ÀÝâxf†W#À_Ž®[ûÑ®,«7µÔ±žŠÁpG#ÂN(ØYØ	n}ëSÚYß]'ôÜXŒ7q¿ùôY<rÂ¹>Ô›29õ)ù‹,Î€h¢¨‰šv•<CÚÑ¶Á�