...
const directRecoveryFunction = `export const fetchAndRestoreCloudBackup = async (email: string, pin: string, overwrite: boolean = false) => {
  ...
  try {
    ...
    try {
      ...
      const candidates = vaultSnap.docs.map(d => ({ id: d.id, data: d.data() || {} }))
        .filter(c => {
          const d = c.data;
          const identities = [d.linked_email, d.linkedEmail, d.email, d.store_id, d.storeId, d.user_id, d.userId, d.owner_id].map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
      ...
      const candidate = candidates.find(c => {
        const d = c.data;
        return (Array.isArray(d.products) && d.products.length > 0) || (Array.isArray(d.contacts) && d.contacts.length > 0) || (Array.isArray(d.expenses) && d.expenses.length > 0) || (Array.isArray(d.transactions) && d.transactions.length > 0) || (Array.isArray(d.purchases) && d.purchases.length > 0) || (d.businessInfo && typeof d.businessInfo === 'object') || (d.business_info && typeof d.business_info === 'object');
      });
    ...
    const collectionMap: Array<[string, string]> = [['products', 'products'], ['customers', 'contacts'], ['expenses', 'expenses'], ['purchases', 'purchases'], ['transactions', 'transactions']];
    for (const [collectionName, resultKey] of collectionMap) {
      try {
        const snap = await getDocs(collection(db, collectionName));
        const rows = snap.docs.map(d => ({ ...d.data(), id: d.data()?.id || d.id }))
          .filter(row => {
            const identities = [row.store_id, row.storeId, row.email, row.linked_email, row.linkedEmail, row.user_id, row.userId, row.owner_id].map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
            return identities.includes(cleanEmail) || (activeUid && identities.includes(String(activeUid).trim().toLowerCase()));
          });
        if (rows.length > 0) {
          result[resultKey] = resultKey === 'transactions' ? rows : rows;
          found = true;
        }
    ...
    try {
      ...
      if (settingsSnap.exists()) {
        ...
        result.business_info = result.businessInfo; found = true;
      } else {
        const businessSnap = await getDocs(collection(db, 'business_info'));
        const matching = businessSnap.docs.map(d => ({ ...d.data(), id: d.data()?.id || d.id }))
          .filter(row => {
            const identities = [row.email, row.linked_email, row.linkedEmail, row.store_id, row.storeId, row.owner_id, row.user_id].map(v => String(v || '').trim().toLowerCase()).filter(Boolean);
            return identities.includes(cleanEmail) || (activeUid && identities.includes(String(activeUid).trim().toLowerCase()));
          }).sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')));
        if (matching.length > 0) { result.businessInfo = { ...(result.businessInfo || result.business_info || {}), ...matching[0] }; result.business_info = result.businessInfo; found = true; }
      }
    ...
    (Array.isArray(result.expenses) ? result.expenses : []).forEach((row: any) => {
      // NEVER derive the business expense date from created_at if an explicit
      // date exists. Date is user-editable business data and must survive reload.
      const preservedDate = row.date ?? row.expenseDate ?? row.created_at;
      const preservedCreatedAt = row.created_at ?? preservedDate;
      canonicalWrites.push(writeCanonical('expenses', { ...row, date: preservedDate, created_at: preservedCreatedAt }));
    });
    ...
    if (Array.isArray(result.transactions)) {
      result.transactions.forEach((row: any) => {
        const flat = row && row.total_amount !== undefined ? row : { id: row?.id, invoice_no: row?.invoiceNo, customer_id: row?.contactId, total_amount: Number(row?.total ?? row?.total_amount) || 0, discount: Number(row?.discount) || 0, vat_rate: Number(row?.vat_rate) || 0, paid_amount: Number(row?.paidAmount ?? row?.paid_amount) || 0, payment_method: row?.paymentMethod || row?.payment_method || 'Cash', signature_svg: row?.customerSignature || row?.signature_svg || null, created_at: row?.date || row?.created_at || new Date().toISOString() };
    ...
    return true;
  } catch (err) { console.warn('[Cloud Restore] Direct Firestore recovery failed:', err); return false; }
};\n\n`;
...
const guards = [['subscribeProducts', 'barakah_products'], ['subscribeCustomers', 'barakah_contacts'], ['subscribeExpenses', 'barakah_expenses'], ['subscribePurchases', 'barakah_purchases'], ['subscribeTransactions', 'barakah_flat_transactions']];
for (const [subscriber, storageKey] of guards) {
  ...
  appSource = appSource.replace(pattern, `$1if (items.length === 0) {\n        try { const cached = localStorage.getItem(getDbKey("${storageKey}", undefined, activeUserId)); if (cached) { const parsed = JSON.parse(cached); if (Array.isArray(parsed) && parsed.length > 0) return; } } catch (_) {}\n      }\n      markRemoteUpdateActive();`);
}
appSource = appSource.replace('if (products.length === 0 && transactions.length === 0 && !isSettingsModified) {', 'if (products.length === 0 && contacts.length === 0 && expenses.length === 0 && transactions.length === 0 && purchases.length === 0 && !isSettingsModified) {');
